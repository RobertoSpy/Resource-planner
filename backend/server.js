...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/utilizatori/me' && req.method === 'GET') {
    ...
    // Folosește verifyToken pentru a verifica autentificarea
    verifyToken(req, res, async () => {
      const userId = req.user?.id; // Obține ID-ul utilizatorului din token-ul JWT
      ...
      try {
        const utilizatorAutentificat = await getUtilizatorAutentificat(req, res, userId); // Funcția din utilizatori.js
    ...
    req.on('end', () => {
      ...
      if (req.method === 'GET') {
        if (req.url === '/api/articole') {
          ...
          console.log(`GET articol cu ID: ${idMatch[1]}`);
        } else {
